...
| --- | --- | --- |
| Toolset IDs on leaf `FunctionToolset`s | Harness Render integration | Fixed by pinned harness commit `e20679c` |
| Model access inside a child task | Harness Render integration | Fixed by pinned harness commit `e20679c` |
...
release. Commit `e20679c` keeps both fixes below in
...
`FunctionToolset` underneath. Commit `e20679c` derives the id when
...
and the `models={...}` entries by id. Commit `e20679c` carries the run's model id
